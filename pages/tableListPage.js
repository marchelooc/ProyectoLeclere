...
export class TableroPage {
  ...
  async agregarLista(nombre) {
    ...
    const lista = this.page.locator('[data-testid="list-header"]', {
      hasText: nombre,
    });
    await expect(lista).toContainText(nombre);
  }
  async renombrarLista(page, nombreActual, nombreNuevo) {
    const listaHeader = page.locator('[data-testid="list-header"]', {
      hasText: nombreActual,
    });
    ...
    const texto = 'textarea[data-testid="list-name-textarea"]';
    await this.page.press(texto, "Enter");
  }
}